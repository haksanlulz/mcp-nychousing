...
### 2026-09-15 — fix round 3: an anchor applied to half its condition, twice

**(a) "Bounded" is not "anchored" — the second half of a two-part condition needs its own boundary.** Round 2 bounded the street's distinctive token on a word boundary and left the type stem free to match anywhere in the line. That is the round-1 lesson reproduced one level down: a street whose name is a PREFIX of a longer street in the same borough carries the stem anyway. Live through the server's own generated `$where`, 590 MORRIS AVENUE in the Bronx reported 1 executed eviction and the matched row was `590 MORRIS PARK AVE`; 632 MORRIS AVENUE the same; 562 MORRIS AVENUE reported 7 where 6 are its own — all three HPD-registered (buildingid 97665 / 97675 / 97662). The stem must now be the next token. What makes this more than a string concat is that SoQL LIKE has no "one or more spaces" quantifier while the column stores runs of them (`3605 SEDGWICK    AVE NUE`), so the runs are enumerated. Re-measured after: 590 / 632 → 0, 562 → 6, while 1520 / 2763 / 2707 / 3605 SEDGWICK hold at 13 / 8 / 7 / 17, 4064 BRONX BLVD at 13, 3704 WHITE PLAINS RD at 1, 1170 OCEAN AVENUE at 4, and 1650 OCEAN / 100 PARK PLACE / 1500 GRAND AVENUE at 0.

**(b) A retry that stops at the first hit leaves the rest unqueried, and nothing said so.** Round 1 gave `dob_building` the house-number variant probe. `tryHouseNumberVariants` returns on the first match — correct — which means a literal spelling holding a few rows hides a hyphenated one holding many, and no note fires: the `matchedVariant` notes need the literal to have FAILED, and the both-zero note needs a double zero. Live on `3h2n-5cm9`, boro `4`, street like `%QUEENS BOULEVARD%`: `9015` returns 12 violations and `90-15` returns 383. The helper now reports which spellings it actually sent, and the note is derived per section, because a section that found nothing already tried them all and naming it would be a false claim.

**(c) The tool whose description says START HERE had no zero-result rescue.** `building_profile` emitted a note only when a house-number variant won. When none matched, a de-hyphenated outer-borough number produced an all-zero profile across every dataset with nothing naming the spellings tried — the shape round 1 closed for `dob_building` and `building_311`, still open in the entry-point tool. `emptyBuildingNote` already existed and is reused, triggered on the profile finding nothing ANYWHERE rather than merely on an unregistered building, so "No rows matched" is never printed beside a non-zero count.

Also closed: PLUTO serves `bbl` float-formatted (`2028800017.00000000`, live for 1520 SEDGWICK AVENUE), so `true_owner` published a BBL that cannot be pasted into ACRIS or DOF and did not match the value the README prints; `verify:mcpb` deleted `NYC_APP_TOKEN` on the line after the loop that set it, which for this single-key manifest cancelled the loop and skipped the one case the server's `${` guard exists for; and the probe's spawned child had no `error` listener, so an ENOENT or a dead-stdin EPIPE escapes the `try` as an uncaught exception and skips the `finally` — reopening the staging-tree leak round 1 closed on the other path. Fault-injected: `FAIL:` line, exit 1, nothing left on disk.

**Docs, third round on the same paragraph.** The README's mutation-probe record claimed one failure and 96 passes beside a stated total of 104 — arithmetically impossible, and left over from when the suite was 97. Re-running the probe shows three failures, not one: the ceiling tests added in round 2 redden under the same mutation. The `toHaveBeenCalled*` site count was stale the same way. **The carry-forward: the docs-sync commit that re-measured the neighbouring figures did not re-run this one, and freshening a stale paragraph's neighbours makes it read checked.** Every figure in that section is re-taken at this round.

### 2026-09-15 — fix round 4: the tool the anchoring work never reached, and the guard nobody wrote

**(a) A per-tool fix is a per-tool fix — the population it did NOT reach is part of the finding.** Rounds 1-3 anchored the house number, then the street token, then the type stem, each time on `building_profile`'s eviction column. `true_owner` was never in that population and got neither half, and it is the tool whose own note calls the recorded deed "the strongest evidence". Its house number rode inside a free substring of PLUTO's combined address: live 2026-09-15 against `64uk-42ks`, `'%17 SEDGWICK AVENUE%'` with `borough='BX'` returns 3817 SEDGWICK AVENUE and 2817 SEDGWICK AVENUE — nothing at house 17 — `found: true`, with `lots[0]`'s owner (a named individual) published as `assessor_owner` and that lot's ACRIS deed and speculation-watch row chased off it. `'%20 QUEENS BOULEVARD%'` returns five lots (114-20, 118-20, 77-20, 109-20, 104-20) and no house 20. **`latest_deed` carries no address of its own, so the substitution is invisible in the field read as the answer** — `assessor_owner_lot_address` now names the lot those three fields describe. Two more holes in the same read: the lot page carried no `$order` while `lots[0]` decides all three, and there was no spelling probe, so `7008 Queens Boulevard` read `found: false` on a real lot that PLUTO stores as `70-08`.

**(b) A guard that only fires on one shape of input is a guard for that shape.** Round 3's street anchor required the type stem to follow the distinctive token — and returned early, before that arm, whenever `streetTypeWord` found no type word. Broadway, Grand Concourse and every Avenue-X-shaped name were therefore back at the round-1 exposure level, and no street of any shape was guarded against a **directional** in front of the distinctive token, which names a different street and is deliberately absent from the type-stem table. Live 2026-09-15 through the server's own `$where`: `building_profile("475", "Broadway", "Manhattan")` reported `evictions_executed: 1` and the matched row was `475 WEST BROADWAY`, both buildings HPD-registered (buildingid 8330 and 43923); 88 Broadway read 3, all West or East Broadway's; 341 and 482 read 1 each, both West Broadway's. With the exclusion — skipped when the caller's own street starts with a directional, so 475 West Broadway still finds its own — 475 / 341 / 482 / 88 Broadway go to 0, 424 Broadway to its own 1, and every regression case holds: 1520 / 2763 / 2707 / 3605 SEDGWICK at 13 / 8 / 7 / 17, 4064 BRONX BLVD 13, 3704 WHITE PLAINS RD 1, 1170 OCEAN AVENUE 4, 562 MORRIS AVENUE 6, 1650 OCEAN / 100 PARK PLACE / 1500 GRAND AVENUE 0.

**(c) A disclosure note is a claim, and round 3's was false in the case it was written for.** The unqueried-spellings note opened "The spelling you passed matched" — untrue whenever a LATER variant matched, which is reachable because `houseNumberVariants("120 15")` returns three entries. In that case the note contradicts the `matchedVariant` note joined into the same string two branches above: one sentence says the passed spelling did not match, the next says it did. Neither of the three tests added with the note used a three-variant input, so the branch was untested as well as wrong. The note now names the spelling that actually stopped each probe, per section.

**Also closed.** The same disclosure was missing entirely from `building_311`, which runs the identical first-match-wins probe — its notes fire on `matchedVariant` (needs the literal to have FAILED) or on a zero, so a literal hit produced neither, while the README's bullet described both tools. Measured for 311 rather than carried over from DOB: rows do not cross spellings (`107-36 QUEENS BOULEVARD` 27 Queens heat rows, `10736 QUEENS BOULEVARD` 0) and both forms are stored (`34 59 AVENUE`, `23 21 ROAD`), but no 311 building was found holding rows under both the way `3h2n-5cm9` does at 12 vs 383 — so the note says what was not QUERIED and does not claim what it would return. The README's address-matching bullet made a population-level claim about "the per-building tools" that `true_owner` and `building_311` never satisfied, on the bullet a reader consults to decide whether a count can be trusted. And `manifest.json`'s `$schema` 404s: mcpb moved from the `anthropics` org to `modelcontextprotocol` and the file sits under `schemas/`, not `dist/` — harmless to the CLI, which validates against its own bundled copies, and a dead link to anyone who follows it.

**Docs, fourth round on the same paragraph, and it is now a pattern rather than an incident.** Round 3 re-took every figure in the README's Testing section; this round moved all of them again (app LOC 2654 → 2782, test LOC 2171 → 2306, suite 113 → 121, mutation probe 3 failed / 110 passed → 3 failed / 118 passed, 31 → 32 `toHaveBeenCalled*` sites). **Any round that touches `server.ts` or `test/` invalidates that whole block**, so re-taking it is not a docs task to be scheduled — it belongs in the same pass as the code.
